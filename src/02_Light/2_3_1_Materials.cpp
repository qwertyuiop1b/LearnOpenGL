...
class Materials {
public:
    Materials(unsigned int width, unsigned int height, const std::string& title)
    : window(std::make_unique<Window>(width, height, title))
    , cubeShader(std::make_unique<Shader>("shaders/02_shaders/2_3_1_Materials.vert", "shaders/02_shaders/2_3_1_Materials.frag"))
    ...
    void run() {
        ...
        while (!window->shouldClose()) {
            ...
            cubeShader->setFloat3("material.ambient", glm::vec3(1.0f, 0.5f, 0.31f));
            cubeShader->setFloat3("material.diffuse", glm::vec3(1.0f, 0.5f, 0.31f));
...
private:
    ...
    void init() {
        ...
        std::vector<VertexAttribute> attrs {
            VertexAttribute{ 0, 3, AttributeType::Float, false, 6 * sizeof(float), (void*)0},
            VertexAttribute{ 1, 3, AttributeType::Float, false, 6 * sizeof(float), (void*)(3 * sizeof(float))}
        };
...
int main() {
    try {
        Materials app(800, 600, "2.3.1 Materials");
        app.run();

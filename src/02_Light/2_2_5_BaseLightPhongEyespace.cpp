...
class BaseLightPhongEyeSpace {
public:
    BaseLightPhongEyeSpace(unsigned int width, unsigned int height, const std::string& title)
    : window(std::make_unique<Window>(width, height, title))
    , lightShader(std::make_unique<Shader>("shaders/02_shaders/2_1_1_Light.vert", "shaders/02_shaders/2_1_1_Light.frag"))
    , cubeShader(std::make_unique<Shader>("shaders/02_shaders/2_2_5_BaseLightPhongEyespace.vert", "shaders/02_shaders/2_2_5_BaseLightPhongEyespace.frag"))
    ...
    void run() {
        ...
        while (!window->shouldClose()) {
            ...
            cubeShader->setFloat3("lightPos", currentLightPos);
...
int main() {
    BaseLightPhongEyeSpace app(800, 600, "2_2_5_BaseLightPhongEyeSpace");
    try {
        app.run();

...
public:
    DirectionalLight(unsigned int width, unsigned int height, const std::string& title) 
    : window(std::make_unique<Window>(width, height, title))
    , cubeShader(std::make_unique<Shader>("shaders/02_shaders/2_5_1_DirectionalLight.vert", "shaders/02_shaders/2_5_1_DirectionalLight.frag"))
    ...
    void run() {
        ...
        while(!window->shouldClose()) {
            ...
            cubeShader->setFloat3("light.direction", glm::vec3(0.f, -1.0f, 0.0));
...
int main() {
    DirectionalLight app(800, 600, "2.5.1.DirectionalLight");
    try {
        app.run();

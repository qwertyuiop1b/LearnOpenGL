...
public:
    ...
    Blending(unsigned int width, unsigned int height, const std::string& title): Application(width, height, title) {
        ...
        // window location
        windowsPos.push_back(glm::vec3(-1.5f,  0.0f, -0.48f));
        windowsPos.push_back(glm::vec3( 1.5f,  0.0f,  0.51f));
        windowsPos.push_back(glm::vec3( 0.0f,  0.0f,  0.7f));
        windowsPos.push_back(glm::vec3(-0.3f,  0.0f, -2.3f));
        windowsPos.push_back(glm::vec3( 0.5f,  0.0f, -0.6f));
    ...
    virtual void render() override {
        ...
        // draw sorted transparent objects last
        // or
        // disable depth test and draw transparent objects first
        // enable depth test and draw opaque objects last
        ...
        // sort window by distance from camera
        // if multiple has the same loaction, only draw the last object?
        // the best way: OIT
        windowsMap.clear();
        for (unsigned int i = 0; i < windowsPos.size(); i++) {
            float distance = glm::length(camera.getEye() - windowsPos[i]);
            windowsMap[distance] = windowsPos[i];
        }

        // draw windows from farthest to closest
        for (auto it = windowsMap.rbegin(); it != windowsMap.rend(); ++it) {
            quadVao.bind();
            windowTexture.bind();
            model = glm::mat4(1.0f);
...
private:
    ...
    std::map<float, glm::vec3> windowsMap;
    ...
    void setup() {
        ...
        windowTexture.loadFromFile("textures/blending_transparent_window.png");
        windowTexture.setParameter(GL_TEXTURE_WRAP_S, static_cast<GLuint>(GL_CLAMP_TO_EDGE));
        windowTexture.setParameter(GL_TEXTURE_WRAP_T, static_cast<GLuint>(GL_CLAMP_TO_EDGE));

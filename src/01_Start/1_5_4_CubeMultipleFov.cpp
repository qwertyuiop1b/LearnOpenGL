...
class CubeMultiple {
public:
CubeMultiple(unsigned int width, unsigned int height, const std::string& title)
    ...
    void run() {
        ...
        while (!window->shouldClose()) {
            ...
            projection = glm::perspective(glm::radians(70.0f), (float)window->getWidth() / (float)window->getHeight(), 0.1f, 100.0f);
...
int main() {
    try {
        CubeMultiple app(800, 600, "1_5_3_CubeMultiple");
        app.run();
